...
exports.AllAdmins=(req,res)=>{
  Admin.find({AdminType:"admin"})
  .then(response=>{
    res.send(response)
  })
}
exports.Pending=(req,res)=>{
  User.find({requestinvestment:true})
    .then(response=>{
      res.send(response)
    })
}
...
exports.verifyinvestor=(req,res)=>{
  ...
  User.findById({_id:req.params.id},(err,user)=>{
    let interval=0
    if(user.planDetails.dataName.split(" ")[0]==="AFI"
     || user.planDetails.dataName.split(" ")[0]==="BFI"){
      interval=12
    }
    else if(user.planDetails.dataName.split(" ")[0]==="AMI"
...
exports.PayLoan=(req,res)=>{
  User.find({repayLoanRequest:true})
  .then(response=>{
    res.send(response)
  })
}
